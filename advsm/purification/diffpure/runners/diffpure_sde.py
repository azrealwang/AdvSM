...
class RevGuidedDiffusion(torch.nn.Module):
    ...
    def image_editing_sample(self, img, bs_id=0, tag=None, return_mid=False, seed=None):
        if seed is None:
            seed = time.time()
        ...
        ctx = torch.enable_grad() if img.requires_grad else torch.no_grad()
        # print(ctx)
        with ctx:
            # if tag is None:
            ...
            for it in range(self.args.sample_step):
                ...
                t_size = self.args.t if return_mid else 1
                ts = torch.linspace(t0, t1, t_size+1).to(self.device)

                x_ = x.view(batch_size, -1)  # (batch_size, state_size)
                if self.args.use_bm:
                    bm = torchsde.BrownianInterval(t0=t0, t1=t1, size=(batch_size, state_size), device=self.device, entropy=int(seed*10000))
