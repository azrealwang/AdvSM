...
from cleverhans.torch.utils import clip_eta, optimize_linear
...
def fast_gradient_method(
    ...
    tg = 0
    for eot_iter in range(eot_iters):
        ...
        gc.collect()
    tg = tg / eot_iters
    optimal_perturbation = optimize_linear(tg, eps, norm)
...
def projected_gradient_descent(
    ...
    while i < nb_iter:
        ...
        _, successful_attack, single_succ = model_fn.eval_attack(
            adv_x.detach(),
            y,
            targeted=targeted,
        )
    ...
    if sanity_checks:
        assert np.all(asserts)
    return adv_x, float(successful_attack), float(single_found)
