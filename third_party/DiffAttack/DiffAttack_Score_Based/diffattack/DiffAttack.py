...
class DiffAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            from .diffattack_base import APGDDiffAttack, PGDDiffAttack, APGDDiffAttackPGDStep
            ...
            self.pgd = PGDDiffAttack(self.model, n_restarts=1, n_iter=100, verbose=False,
                eps=self.epsilon, norm=self.norm, eot_iter=1, seed=self.seed,
                device=self.device)
            self.apgd_pgd = APGDDiffAttackPGDStep(self.model, n_restarts=1, n_iter=100, verbose=False,
                eps=self.epsilon, norm=self.norm, eot_iter=1, seed=self.seed,
                device=self.device, logger=self.logger, args=args)
            '''
            ...
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    if len(x.shape) == 3:
                        x.unsqueeze_(dim=0)
                    # run attack
                    if attack == 'apgd-pgd-ce':
                        self.apgd_pgd.seed = self.get_seed()
                        adv_curr = self.apgd_pgd.perturb(x, y)
                    elif attack == 'pgd-ce':
                        self.pgd.seed = self.get_seed()
                        adv_curr = self.pgd.perturb(x, y)
                    elif attack == 'apgd-ce':
                    ...
                    # Always keep the latest adversarial candidate for every sample in this batch,
                    # even if classification has not changed (attack "failed" under the budget).
                    # robust_flags/y_adv still track success for reporting; x_adv stores best adv.
                    x_adv[batch_datapoint_idcs] = adv_curr.detach().to(x_adv.device)
                    y_adv[batch_datapoint_idcs] = output.detach().to(y_adv.device)
